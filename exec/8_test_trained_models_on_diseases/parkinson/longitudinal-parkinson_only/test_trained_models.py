...
from hgsprediction.save_results import parkinson_save_spearman_correlation_results
from hgsprediction.save_results import parkinson_save_hgs_predicted_results
...
if mri_status == "mri+nonmri":
    df_longitudinal_mri = parkinson_load_data.load_preprocessed_data(population, "mri", session_column, parkinson_cohort)
    df_longitudinal_nonmri = parkinson_load_data.load_preprocessed_data(population, "nonmri", session_column, parkinson_cohort)
    df_longitudinal = pd.concat([df_longitudinal_mri, df_longitudinal_nonmri])
else:
    df_longitudinal = parkinson_load_data.load_preprocessed_data(population, mri_status, session_column, parkinson_cohort)
...
for parkinson_subgroup in ["pre-parkinson", "post-parkinson"]:
    ...
    if visit_session == "1":
        ...
        prefix = f"1st_{parkinson_subgroup}_"
...
if df_merged['1st_pre-parkinson_gender'].astype(float).equals((df_merged['1st_post-parkinson_gender'].astype(float))):
    ...
    df_merged = df_merged.drop(columns=['1st_pre-parkinson_gender', '1st_post-parkinson_gender'])
...
for parkinson_subgroup in ["pre-parkinson", "post-parkinson"]:
    df_extracted = df_merged[[col for col in df_merged.columns if parkinson_subgroup in col]]
    ...
    parkinson_save_spearman_correlation_results(
        df_corr,
        df_pvalue,
        population,
        mri_status,
