...
df_pre.set_index("SubjectID", inplace=True)
print("===== Done! =====")
embed(globals(), locals())
...
df_1_pre_f = df_pre[df_pre['31-0.0']==0.0]
df_1_pre_m = df_pre[df_pre['31-0.0']==1.0]

f_days = abs(df_1_pre_f["1_pre_days"])
f_hgs_L = df_1_pre_f["1_pre_left_hgs"]
f_hgs_R = df_1_pre_f["1_pre_right_hgs"]
f_hgs_LR = df_1_pre_f["1_pre_left_hgs"] + df_1_pre_f["1_pre_right_hgs"]

m_days = abs(df_1_pre_m["1_pre_days"])
m_hgs_L = df_1_pre_m["1_pre_left_hgs"]
m_hgs_R = df_1_pre_m["1_pre_right_hgs"]
m_hgs_LR = df_1_pre_m["1_pre_left_hgs"] + df_1_pre_m["1_pre_right_hgs"]
...
ax[0].set_xlabel('Actual Pre Left HGS', fontsize=20, fontweight="bold")
ax[0].set_ylabel('Post-stroke days', fontsize=20, fontweight="bold")

ax[0].set_title("Post-stroke days vs Actual Pre Left HGS", fontsize=15, fontweight="bold", y=1.02)
...
ax[1].set_xlabel('Actual Pre Right HGS', fontsize=20, fontweight="bold")
ax[1].set_ylabel('Post-stroke days', fontsize=20, fontweight="bold")

ax[1].set_title("Post-stroke days vs Actual Pre Right HGS", fontsize=15, fontweight="bold", y=1.02)
...
ax[2].set_xlabel('Actual Pre (Left+Right) HGS', fontsize=20, fontweight="bold")
ax[2].set_ylabel('Post-stroke days', fontsize=20, fontweight="bold")

ax[2].set_title("Post-stroke days vs Actual Pre (Left+Right) HGS", fontsize=15, fontweight="bold", y=1.02)
...
plt.suptitle(f"Post-stroke days vs Actual Pre HGS - Males({len(df_1_pre_m)})", fontsize=20, fontweight="bold", y=0.9)

plt.show()
plt.savefig(f"correlate_pre_days_actual_hgs_storke_Males.png")
plt.close()
# ##############################################################################################################################################################
##############################################################################################################################################################
# Create the actual HGS vs predicted HGS plot for females and fefemales separately
...
# ax2.ylabel('Correlation of LCOR with Predicted HGS', fontsize=25, fontweight="bold")
ax[2].set_xlabel('Actual Pre (Left+Right) HGS', fontsize=20, fontweight="bold")
ax[2].set_ylabel('Pre-stroke days', fontsize=20, fontweight="bold")

ax[2].set_title("Pre-stroke days vs Actual Pre (Left+Right) HGS", fontsize=15, fontweight="bold", y=1.02)
ax[2].text(xmax2 - 0.05 * xmax2, ymax2 - 0.01 * ymax2, text2, verticalalignment='top',
         horizontalalignment='right', fontsize=18, fontweight="bold")
...
plt.suptitle(f"Post-stroke days vs Actual Pre HGS - Females({len(df_1_pre_f)})", fontsize=20, fontweight="bold", y=0.9)

plt.show()
plt.savefig(f"correlate_pre_days_actual_hgs_storke_Females.png")
plt.close()
