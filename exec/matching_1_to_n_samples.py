...
##############################################################################
    ...
    sns.regplot(data=df_female_stroke, x=f"{target}_actual", y=f"{target}_predicted", ax=ax[1][axj], scatter_kws={"color": "#a851ab"}, line_kws={"color": "darkgrey"})
    sns.regplot(data=df_male_stroke, x=f"{target}_actual", y=f"{target}_predicted", ax=ax[1][axj], scatter_kws={"color": "#005c95"}, line_kws={"color": "darkgrey"})
    
    # sns.scatterplot(data=df_both_stroke, x=f"{target}_actual", y=f"{target}_predicted", hue="gender", ax=ax[1][axj], palette = custom_palette, legend=False)
    ax[1][axj].set_xlim(math.floor(df_both_stroke[f"{target}_actual"].min()/10)*10, math.ceil(df_both_stroke[f"{target}_actual"].max()/10)*10+10)
    ax[1][axj].set_ylim(math.floor(df_both_stroke[f"{target}_predicted"].min()/10)*10, math.ceil(df_both_stroke[f"{target}_predicted"].max()/10)*10+10)
...
plt.tight_layout()
plt.show()
plt.savefig(f"control_1_10_all_samples_predicted_{target}_excluding_1872273_separate_no_outliers_mri.png")
plt.close()
print("===== Done! =====")
embed(globals(), locals())
#     custom_palette = sns.color_palette(['#a851ab', '#005c95'])  # You can use any hex color codes you prefer
...
#     plt.show()
#     plt.savefig(f"control_1_10_samples_predicted_{target}_excluded_1872273_{stroke_cohort}.png")
#     plt.close()
...
# plt.show()
# plt.savefig(f"strok_predicted_{target}_excluded_1872273_{stroke_cohort}.png")
# plt.close()
