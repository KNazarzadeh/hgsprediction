...
last_corr_both_gender = df_sorted_both_gender.tail(int(n_last_regions))
last_corr_female = df_sorted_female.tail(int(n_last_regions))
last_corr_male = df_sorted_male.tail(int(n_last_regions))
...
last_corr_both_gender['Normalized_Correlation'] = last_corr_both_gender['average_correlations'].clip(lower=vmin, upper=vmax)
normalized_values_both_gender = norm(last_corr_both_gender['Normalized_Correlation'])

last_corr_male['Normalized_Correlation'] = last_corr_male['correlations_values'].clip(lower=vmin, upper=vmax)
normalized_values_male = norm(last_corr_male['Normalized_Correlation'])

last_corr_female['Normalized_Correlation'] = last_corr_female['correlations_values'].clip(lower=vmin, upper=vmax)
normalized_values_female = norm(last_corr_female['Normalized_Correlation'])
...
ax[0].scatter(
    x=last_corr_both_gender['regions'], 
    y=last_corr_both_gender['average_correlations'], 
    c=last_corr_both_gender['average_correlations'],  # Use correlation values for coloring
...
ax[0].vlines(x=last_corr_both_gender['regions'], 
          ymin=0,                          # From the x-axis (y=0)
          ymax=last_corr_both_gender['average_correlations'], 
          color='darkgrey', 
          linewidth=4)
###############################################################################
...
ax[1].scatter(
    x=last_corr_male['regions'], 
    y=last_corr_male['correlations_values'], 
    c=last_corr_male['correlations_values'],  # Use correlation values for coloring
    cmap=cmap_custom,                        # Apply the custom colormap
    norm=norm,                               # Normalize between vmin and vmax
    zorder=5, 
    s=300
...
# Draw vertical lines from each scatter point down to the x-axis
ax[1].vlines(x=last_corr_male['regions'], 
          ymin=0,                          # From the x-axis (y=0)
          ymax=last_corr_male['correlations_values'], 
          color='darkgrey', 
          linewidth=4)
###############################################################################
...
# norm = colors.BoundaryNorm(boundaries=custom_ticks, ncolors=cmap_custom.N)
ax[2].scatter(
    x=last_corr_female['regions'], 
    y=last_corr_female['correlations_values'], 
    c=last_corr_female['correlations_values'],  # Use correlation values for coloring
    cmap=cmap_custom,                        # Apply the custom colormap
    norm=norm,                               # Normalize between vmin and vmax
    zorder=5, 
    s=300
...
# Draw vertical lines from each scatter point down to the x-axis
ax[2].vlines(x=last_corr_female['regions'], 
          ymin=0,                          # From the x-axis (y=0)
          ymax=last_corr_female['correlations_values'], 
          color='darkgrey', 
          linewidth=4)
...
ax[0].set_xticks(np.arange(len(last_corr_both_gender)))
ax[0].set_xticklabels(last_corr_both_gender['regions'], rotation=45, ha='right', fontsize=24)
...
# ax[0].set_xlim(0.5, len(last_corr_both_gender) + 0.5)

ax[1].set_xticks(np.arange(len(last_corr_male)))
ax[1].set_xticklabels(last_corr_male['regions'], rotation=45, ha='right', fontsize=24)
ax[1].set_xlim(0.5, len(last_corr_male) + 0.5)

ax[2].set_xticks(np.arange(len(last_corr_female)))
ax[2].set_xticklabels(last_corr_female['regions'], rotation=45, ha='right', fontsize=24)
ax[2].set_xlim(0.5, len(last_corr_female) + 0.5)
...
if corr_target == "hgs_corrected_delta":
    ...
    ystep = 0.04
    # Remove the last and right spines (borders)
...
###############################################################################
fig.suptitle(f'last {n_last_regions} Regions with Highest Absolute Correlations', fontsize=24, y=1)
###############################################################################
plt.tight_layout()
# Show the plot
plt.show()
plt.savefig(f"all_genders_delta_hgs_regions_{corr_target}_{n_last_regions}_last_regions.png")
