...
if mri_status == "mri+nonmri":
    df_pre_mri = depression_load_data.load_preprocessed_data(population, "mri", session_column, depression_cohort)
    df_pre_nonmri = depression_load_data.load_preprocessed_data(population, "nonmri", session_column, depression_cohort)
    
    df_pre = pd.concat([df_pre_mri, df_pre_nonmri])
else:
    
    df_pre = depression_load_data.load_preprocessed_data(population, mri_status, session_column, depression_cohort)

# Define the string to check for in column names
string_to_remove = 'post-depression'
...
df_extracted = depression_extract_data.extract_data(df_pre, depression_cohort, visit_session, features, target)
