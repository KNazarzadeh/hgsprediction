...
"""Perform different preprocessing on parkinson."""
...
###############################################################################
class ParkinsonMainDataPreprocessor:
...
###############################################################################
    def remove_missing_parkinson_dates(self, df):
        """ Drop all subjects who has no date of parkinson and
        ...
        df : pandas.DataFrame
            DataFrame of data specified.   
        """
        df = df[(~df['131022-0.0'].isna()) & (df['131022-0.0'] != "1900-01-01")]
...
# between parkinson date and the attendence dates (for each session) 
# To see each subject had parkinson before/after parkinson
# to find the subject's parkinson cohort (Pre-, Post- or Longitudinal)
###############################################################################
...
###############################################################################
    def preprocess_parkinson_df(self, df):
        """Use the parkinson dataframe. 
        ...
        post_sorted_columns.columns = [f"1st_{parkinson_cohort}_session",
                                        f"2nd_{parkinson_cohort}_session",
                                        f"3rd_{parkinson_cohort}_session",
                                        f"4th_{parkinson_cohort}_session"]
        ...
        # Combine selected rows from 'df' with sorted columns from 'sorted_columns' to create 'post_parkinson_visits_df'
        post_parkinson_visits_df = pd.concat([df[df.index.isin(post_sorted_columns.index)], post_sorted_columns], axis=1)
        ##################################################################        
        ############### Extract All Pre parkinson Subjects #################
        ##################################################################
        parkinson_cohort = "pre-parkinson"
        # Drop rows where values in all four columns of followup_days 
        # are greather than 0 (Drop All Post-parkinson subjects)
        # And keep rows with at least one value (< 0)
        ...
        # Set custom column names for the sorted_columns DataFrame
        pre_sorted_columns.columns = [f"1st_{parkinson_cohort}_session",
                                        f"2nd_{parkinson_cohort}_session",
                                        f"3rd_{parkinson_cohort}_session",
                                        f"4th_{parkinson_cohort}_session"]
        ...
        # Combine selected rows from 'df' with sorted columns from 'sorted_columns' to create 'pre_parkinson_visits_df'
        pre_parkinson_visits_df = pd.concat([df[df.index.isin(pre_sorted_columns.index)], pre_sorted_columns], axis=1)
        
        ##################################################################        
        ############### Extract Longitudinal parkinson Subjects #############
        ##################################################################
        # The intersection between pre and post dataframes of parkinson
        # will be the longitudinal dataframe.
        # Find duplicate index values
        duplicate_index = pre_parkinson_visits_df.index.intersection(post_parkinson_visits_df.index)
        # Find duplicate index values on the original DataFrame
        df_longitudinal_parkinson = df[df.index.isin(duplicate_index)]
        # List of new pre_visits column names
        pre_visits_columns = ["1st_pre-parkinson_session",
                              "2nd_pre-parkinson_session",
                              "3rd_pre-parkinson_session",
                              "4th_pre-parkinson_session"]
        # List of new post_visits column names
        post_visits_columns = ["1st_post-parkinson_session",
                               "2nd_post-parkinson_session",
                               "3rd_post-parkinson_session",
                               "4th_post-parkinson_session"]
        # Join pre_visits columns from df_all_pre_parkinson_visits into longitudinal_parkinson_df    
        df_longitudinal_parkinson = df_longitudinal_parkinson.join(pre_parkinson_visits_df[pre_parkinson_visits_df.index.isin(duplicate_index)][pre_visits_columns])
        # Join post_visits columns from df_all_post_parkinson_visits into longitudinal_parkinson_df
        df_longitudinal_parkinson = df_longitudinal_parkinson.join(post_parkinson_visits_df[post_parkinson_visits_df.index.isin(duplicate_index)][post_visits_columns])
        ##################################################################        
        ############### Merge All 3 types of sub-type parkinson #############
        ##################################################################
        # Merge All 3 types of sub-type parkinson dataframes 
        # while excluding duplicates from pre-parkinson and post-parkinson visits,
        # and include the longitudinal parkinson dataframe.
        merged_df = pd.concat([pre_parkinson_visits_df[~pre_parkinson_visits_df.index.isin(duplicate_index)],
                               post_parkinson_visits_df[~post_parkinson_visits_df.index.isin(duplicate_index)],
                               df_longitudinal_parkinson])
...
###############################################################################
    def validate_handgrips(self, df, session_column):
        ...
        if df[session_column].isna().sum() < len(df):
            ...
            # Calling the modules
            df = self.remove_missing_hgs_pre_post_parkinson(df, session_column)
            hgs_dominant = session_column.replace(substring_to_remove, "hgs_dominant")
            # ------------------------------------
...
###############################################################################
    def remove_missing_hgs_pre_post_parkinson(self, df, session_column):
...
###############################################################################
    def extract_post_parkinson_df(self, df, mri_status):
        """Extract the post parkinson dataframe from the parkinson dataframe. 
        ...
        # for only Post-parkinson subjects
        post_parkinson_df = df[mask]
        
        return post_parkinson_df

###############################################################################
    def extract_pre_parkinson_df(self, df, mri_status):
        """Extract the pre parkinson dataframe from the parkinson dataframe. 
        ...
        # Apply the mask to the DataFrame to get the desired rows
        # for only Pre-parkinson subjects
        pre_parkinson_df = df[mask]
        
        return pre_parkinson_df

###############################################################################
    def extract_longitudinal_parkinson_df(self, df, mri_status):
        """Extract the longitudinal dataframe from the parkinson dataframe. 
        ...
        df : pandas.DataFrame
            DataFrame of data specified.
        """
        df_pre_parkinson = self.extract_pre_parkinson_df(df, mri_status)
        df_post_parkinson = self.extract_post_parkinson_df(df, mri_status)

        # The intersection between pre and post dataframes of parkinson
        # will be the longitudinal dataframe.
        # Find duplicate index values
        merged_df = pd.concat([df_pre_parkinson, df_post_parkinson])
        # Find duplicate index values on the original DataFrame
        longitudinal_parkinson_df = df[~df.index.isin(merged_df.index)]
        
        return longitudinal_parkinson_df
...
###############################################################################
class parkinsonValidateDataPreprocessor:
    def __init__(self, 
                 df, 
                 mri_status,
                 parkinson_cohort, 
        ...
        self.parkinson_cohort = parkinson_cohort
        self.visit_session = visit_session
        
        assert isinstance(df, pd.DataFrame), "df must be a dataframe!"
        assert isinstance(mri_status, str), "mri_status must be a string!"
        assert isinstance(parkinson_cohort, str), "parkinson_cohort must be a string!"
        assert isinstance(visit_session, int), "visit_session must be a string!"

        if visit_session == 1:
            self.session_column = f"1st_{parkinson_cohort}_session"
        elif visit_session == 2:
            self.session_column = f"2nd_{parkinson_cohort}_session"
        elif visit_session == 3:
            self.session_column = f"3rd_{parkinson_cohort}_session"
        elif visit_session == 4:
            self.session_column = f"4th_{parkinson_cohort}_session"
################################ EXTRACT DATA ##############################
# The main goal of data validation is to verify that the data is 
# accurate, reliable, and suitable for the intended analysis.
###############################################################################
...
###############################################################################
    def validate_handgrips(self, df):
        ...
        if df[session_column].isna().sum() < len(df):
            # Calculate Dominant HGS by
            # Calling the modules
            df = self.remove_missing_hgs_pre_post_parkinson(df)
...
###############################################################################
    def remove_missing_hgs_pre_post_parkinson(self, df):
