...
##############################################################################
    # Assuming you have imported the necessary libraries and have your data loaded in 'df'
    # 'x' and 'y' are the column names you want to plot
    # 'gender' is the column used for hue
    # Define the figure size (width, height) in inches
    custom_palette = {1: 'blue', 0: 'lightcoral'}
    # Create the jointplot with KDE and hue
    g1 = sns.jointplot(x=x, y=y, data=df, hue="gender", palette=custom_palette)
    ...
    both_gender_corr = spearmanr(df[y], df[x])[0]
    female_corr = spearmanr(df_female[y], df_female[x])[0]
    male_corr = spearmanr(df_male[y], df_male[x])[0]
    
    # Access the automatically shown legend
    auto_legend = g1.ax_joint.get_legend()
    # Customize the legend labels if needed
    for text in auto_legend.get_texts():
        if text.get_text() == '0':
            text.set_text(f"Female, r = {female_corr:.3f}")
        elif text.get_text() == '1':
            text.set_text(f"Male, r = {male_corr:.3f}")
    
    # Set the title
    g1.fig.suptitle(f"{y_label} vs {x_label} \n Features={feature_label}, Target={target_label} \n (Females={len(df_female)}, Males={len(df_male)})", fontsize=10, fontweight="bold")
    # Adjust the subplot spacing to move the title to the main plot area
    # Shift the plot to the right by adjusting the left margin
    g1.fig.subplots_adjust(top=0.9, right=0.95, left=0.15)  
    # Set x-label and y-label
    g1.set_axis_labels(xlabel=f"{x_label}", ylabel=f"{y_label}", fontsize=20, fontweight="bold")
    # Set x and y ticks fontsize
    g1.ax_joint.tick_params(axis='x', labelsize=12)
    g1.ax_joint.tick_params(axis='y', labelsize=12)
    
    xmax = np.max( g1.ax_joint.get_xlim())
    ymax = np.max( g1.ax_joint.get_ylim())
...
############################################################################## 
# Create the actual HGS vs predicted HGS plot for females and fefemales separately
def plot_hgs_correlations_kde_plot(df, 
                                    x,
                                    y,
                                    population,
                                    mri_status,
