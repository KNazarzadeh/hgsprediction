...
##############################################################################
    ...
    ##############################################################################
        ...
        for gender in ["Female", "Male"]:
            ...
            for k in range(1, n + 1):
                ...
                if gender == "Female":
                    ...
                    corr_female_parkinson = spearmanr(df_female_parkinson[f"{target}_predicted"], df_female_parkinson[f"{target}_true"])[0]
                elif gender == "Male":                
                    ...
                    corr_male_parkinson = spearmanr(df_male_parkinson[f"{target}_predicted"], df_male_parkinson[f"{target}_true"])[0]
            print(matched_data)
            print(matched_pairs)
        ...
        corr_parkinson = spearmanr(df_both_parkinson[f"{target}_predicted"], df_both_parkinson[f"{target}_true"])[0]
        ...
        df_both_parkinson.rename(columns={f'{target}_true':"true", f"{target}_predicted":"predicted", f"{target}_(true-predicted)": "delta"}, inplace=True)
        if target == "hgs_L+R":
            if parkinson_cohort == "pre-parkinson":
...
a = df_anova[["disease", "gender", "predicted", "hgs_target", "parkinson_cohort"]]
...
b["group"].replace(1, "parkinson", inplace=True)
...
df_parkinson_anova = [df_left_parkinson_pre["predicted"], df_left_parkinson_post["predicted"], df_right_parkinson_pre["predicted"], df_right_parkinson_post["predicted"], df_l_r_parkinson_pre["predicted"], df_l_r_parkinson_post["predicted"]]
...
df_parkinson_pre_anova = [df_left_parkinson_pre["predicted"], df_right_parkinson_pre["predicted"], df_l_r_parkinson_pre["predicted"]]
df_parkinson_post_anova = [df_left_parkinson_post["predicted"], df_right_parkinson_post["predicted"], df_l_r_parkinson_post["predicted"]]
