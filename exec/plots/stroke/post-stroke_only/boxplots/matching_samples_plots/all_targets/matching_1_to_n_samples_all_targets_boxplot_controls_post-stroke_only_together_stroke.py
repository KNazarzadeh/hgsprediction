...
##############################################################################
    ...
    for stroke_cohort in ["post-stroke"]:
        if stroke_cohort == "post-stroke":
            df = df_post.copy()
    ...
    ##############################################################################
        ...
        for gender in ["Female", "Male"]:
            ...
            data.loc[:, 'propensity_scores'] = propensity_scores
...
df_stroke_together = df_both_stroke_post
...
for y_axis in ["actual", "predicted", "delta"]:
    ...
    for i, hgs_target_stroke_cohort in enumerate(["HGS Left-post", "HGS Right-post", "HGS L+R-post"]):
    ...
    for i, x_box_pos in enumerate(np.arange(0,len(xticks_positios_array)-1,2)):
    ...
    plt.show()
    plt.savefig(f"boxplot_samples_{session_column}_{y_axis}_{population}_{feature_type}_hgs_both_gender_controls_Stroke.png")
    plt.close()
    # print("===== Done! =====")
    # embed(globals(), locals())
###############################################################################
    ...
    plt.show()
    plt.savefig(f"boxplot_samples_{session_column}_{y_axis}_{population}_{feature_type}_hgs_separate_gender_separated_Stroke_Female.png")
    plt.close()
...
###############################################################################
    ...
    plt.show()
    plt.savefig(f"boxplot_samples_{session_column}_{y_axis}_{population}_{feature_type}_hgs_separate_gender_separated_Stroke_Male.png")
    plt.close()
