...
def extract_data(df, depression_cohort, visit_session, features, target):
    ...
    df = rename_column_names(df, depression_cohort, visit_session) 
    
    return df

def rename_column_names(df, depression_cohort, visit_session):
    
    if visit_session == "1":
        prefix = f"1st_{depression_cohort}_"
    elif visit_session == "2":
        prefix = f"2nd_{depression_cohort}_"
    elif visit_session == "3":
        prefix = f"3rd_{depression_cohort}_"
    elif visit_session == "4":
        prefix = f"4th_{depression_cohort}_"
