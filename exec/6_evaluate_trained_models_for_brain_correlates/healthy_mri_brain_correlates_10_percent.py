...
sorted_df_female = df_intersected_female.sort_values(by='1st_scan_hgs_L+R_(actual-predicted)', ascending=False)
sorted_df_male = df_intersected_male.sort_values(by='1st_scan_hgs_L+R_(actual-predicted)', ascending=False)
...
# sns.regplot(data=df_female_corr, x=df_female_corr.T['1st_scan_hgs_L+R_actual'], y=df_female_corr.T['1st_scan_hgs_L+R_(actual-predicted)'], ax=ax[0], color=custom_palette_female, marker="$\circ$", scatter_kws={"s":50}, line_kws={"color": "red"})
# sns.regplot(data=df_male_corr, x=df_male_corr.T['1st_scan_hgs_L+R_actual'], y=df_male_corr.T['1st_scan_hgs_L+R_(actual-predicted)'], ax=ax[1], color= custom_palette_male, marker="$\circ$", scatter_kws={"s":50}, line_kws={"color": "#069AF3"})
...
g = sns.jointplot(data=df_combined, x='1st_scan_hgs_L+R_actual', y='1st_scan_hgs_L+R_(actual-predicted)', hue="gender", palette=custom_palette,  marker="$\circ$", s=120)

for gender_type, gr in df_combined.groupby('gender'):
    slope, intercept, r_value, p_value, std_err = linregress(gr['1st_scan_hgs_L+R_actual'], gr['1st_scan_hgs_L+R_(actual-predicted)'])
    if gr['gender'].any() == 0:
        female_corr = pearsonr(gr['1st_scan_hgs_L+R_(actual-predicted)'], gr['1st_scan_hgs_L+R_actual'])[0]
        female_R2 = r2_score(gr['1st_scan_hgs_L+R_actual'], gr['1st_scan_hgs_L+R_(actual-predicted)'])
        print(female_corr)
        print("female_r2=", female_R2)
    elif gr['gender'].any() == 1:
        male_corr = pearsonr(gr['1st_scan_hgs_L+R_(actual-predicted)'], gr['1st_scan_hgs_L+R_actual'])[0]
        male_R2 = r2_score(gr['1st_scan_hgs_L+R_actual'], gr['1st_scan_hgs_L+R_(actual-predicted)'])
        print(male_corr)
        print("male_r2=", male_R2)
        
    sns.regplot(x=gr['1st_scan_hgs_L+R_actual'], y=gr['1st_scan_hgs_L+R_(actual-predicted)'], ax=g.ax_joint, scatter=False, color=custom_palette[gender_type])
...
g.ax_joint.set_xlabel("True GMV correlation", fontsize=12, fontweight="bold")
g.ax_joint.set_ylabel("Delta GMV correlation", fontsize=12, fontweight="bold")
...
plt.show()
plt.savefig(f"correlate_mri.png")
plt.close()
