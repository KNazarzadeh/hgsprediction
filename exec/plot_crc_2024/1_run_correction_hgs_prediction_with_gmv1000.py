...
cort_img = datasets.fetch_atlas_schaefer_2018(n_rois=1000, yeo_networks=7, resolution_mm=1, data_dir=download_path, resume=True, verbose=1)
nii_gmv_path = '/data/project/stroke_ukb/knazarzadeh/project_hgsprediction/brain_imaging_data/Schaefer2018_1000Parcels_7Networks_order_FSLMNI152_1mm.nii'
cort_img = nib.load(nii_gmv_path)
# Tian
nii_tian_file_path = '/data/project/stroke_ukb/knazarzadeh/project_hgsprediction/brain_imaging_data/Tian_Subcortex_S4_3T.nii'
...
for gender in ["female", "male"]:
    ...
    for cnfd_idx, cnfd in enumerate(list(df_tmp.columns)[:2]):
        ...
        # cortical
        cort_corr = df_tmp.iloc[:1000, cnfd_idx-1].copy()
        cort_corr_newidx = cort_corr.copy()
        cort_corr_newidx.reset_index()
        # subcortical Tian
        sub_corr = df_tmp.iloc[1000:1054, cnfd_idx-1].copy()
        sub_corr_newidx = sub_corr.copy()
        sub_corr_newidx.reset_index()
        # cerebellar SUID
        cer_corr = df_tmp.iloc[1054:, cnfd_idx-1].copy()
        ...
        folder_path = f"/data/project/stroke_ukb/knazarzadeh/project_hgsprediction/brain_imaging_data/brain_plots/schaefer{schaefer}/{gender}/{brain_plot}"
        cort_corr_atlas_fname = os.path.join(folder_path, f"schaefer{schaefer}_correlation-GMV-{cnfd}.nii")
