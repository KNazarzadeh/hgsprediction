...
if mri_status == "mri+nonmri":
    df_pre_mri = stroke_load_data.load_preprocessed_data(population, "mri", session_column, stroke_cohort)
    df_pre_nonmri = stroke_load_data.load_preprocessed_data(population, "nonmri", session_column, stroke_cohort)
    df_pre = pd.concat([df_pre_mri, df_pre_nonmri])
else:
    df_pre = stroke_load_data.load_preprocessed_data(population, mri_status, session_column, stroke_cohort)
print("===== Done! =====")
embed(globals(), locals())
# Define the string to check for in column names
string_to_remove = 'post-stroke'
...
df_extracted = stroke_extract_data.extract_data(df_pre, stroke_cohort, visit_session, features, target)
