...
def plot_bar_with_scatter(data, x, y, corr_target, gender, n_regions_survived, color):
    ...
    plt.tight_layout()
    plt.show()
    plt.savefig(f"corr_gmv_without_TIV_and_true_hgs_schaefer{schaefer}_{stats_correlation_type}_{model_name}_{corr_target}_{target}_{gender}.png")  # Save the plot as a PNG file
# print("===== Done! =====")
# embed(globals(), locals())
##############################################################################
##############################################################################
