...
##############################################################################
# plot
    for cov, feature in enumerate(covariates):
        exclude_features = [item for item in covariates if item != feature]
        ...
        for gender in ["Female", "Male"]:
            ...
            propensity_model.fit(data[feature].values.reshape(-1, 1), data['disease'])
            propensity_scores = propensity_model.predict_proba(data[feature].values.reshape(-1, 1))[:, 1]
            ...
            knn.fit(control_group[feature].values.reshape(-1, 1))

            # Find the nearest neighbors for each diseaseed unit
            distances, indices = knn.kneighbors(disease_group[feature].values.reshape(-1, 1))
            ...
            # Access all columns except those in new_list
            columns_to_access = [col for col in disease_group.columns if col not in exclude_features]
            # Use the matched pairs to create the matched data
            matched_data = disease_group[columns_to_access].reset_index(drop=True).join(control_group[columns_to_access].iloc[indices.flatten()].reset_index(drop=True), lsuffix="_disease", rsuffix="_control")
            ...
            matched_data.to_csv(f"{covariates[cov]}_{stroke_cohort}_{gender}_{target}_without_hgs", sep=',', index=False)
##############################################################################
##############################################################################
            y1 = np.full((len(matched_data)), 0.6)
            ax[axi].scatter(matched_data[feature+"_disease"], y1, s=100, edgecolors='black', c='white')
            ...
            ax[axi].scatter(matched_data[feature+"_control"], y2, s=100, edgecolors='black', c='white')
            ...
            ax[axi].plot([df_y1[feature], df_y2[feature]], [df_y1["yaxis"], df_y2["yaxis"]], linestyle='-')
            ...
            ax[axi].text(x_axis_mean, 0.6 + 0.08, text1, horizontalalignment='center', fontsize=12, fontweight="bold")
            ax[axi].text(x_axis_mean, 0.2 - 0.08, text2, horizontalalignment='center', fontsize=12, fontweight="bold")
        ...
        fig.suptitle(f"Distribution of Features-{stroke_cohort}\nTarget={target_label}-Feature={covariates[cov].upper()}", fontsize=16, fontweight="bold")
        plt.show()
        plt.savefig(f"{covariates[cov]}_FT_distribution_{target}_{stroke_cohort}_{gender}_without_hgs.png")
        plt.close()
