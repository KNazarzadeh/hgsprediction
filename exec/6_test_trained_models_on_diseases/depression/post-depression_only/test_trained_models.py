...
if mri_status == "mri+nonmri":
    df_post_mri = depression_load_data.load_preprocessed_data(population, "mri", session_column, depression_cohort)
    df_post_nonmri = depression_load_data.load_preprocessed_data(population, "nonmri", session_column, depression_cohort)
    df_post = pd.concat([df_post_mri, df_post_nonmri])
else:
    df_post = depression_load_data.load_preprocessed_data(population, mri_status, session_column, depression_cohort)

# Define the string to check for in column names
string_to_remove = 'pre-depression'
...
df_extracted = depression_extract_data.extract_data(df_post, depression_cohort, visit_session, features, target)
...
session_column = f"1st_{depression_cohort}_only_session"
print("===== Done! =====")
embed(globals(), locals())
