...
register_scorer("spearman_corr", spearman_scorer)
###############################################################################
...
df_test_spearman_r_score = scores_trained.set_index(
    ['repeat', 'fold'])['test_spearman_corr'].unstack()
df_test_spearman_r_score.index.name = 'Repeats'
df_test_spearman_r_score.columns.name = 'K-fold splits'

print(df_test_spearman_r_score)

df_test_mae_r_score = scores_trained.set_index(
    ['repeat', 'fold'])['test_neg_mean_absolute_error'].unstack()
df_test_mae_r_score.index.name = 'Repeats'
df_test_mae_r_score.columns.name = 'K-fold splits'

print(df_test_mae_r_score * -1)
###############################################################################
df_prediction_r2_scores = pd.DataFrame()
df_prediction_pearson_scores = pd.DataFrame()
df_prediction_spearman_scores = pd.DataFrame()
df_prediction_mae_scores = pd.DataFrame()
...
for idx, (train_val_index, validation_index) \
    ...
    # Calculate 'r2_score', 'pearson', 'spearman' and 'mean absolute error' scoring
    ...
    df_prediction_spearman_scores.loc[f'{repeat}', f'{fold}'] = spearman_score
    df_prediction_mae_scores.loc[f'{repeat}', f'{fold}'] = mae_score
...
df_prediction_spearman_scores.index.name = 'Repeats'
df_prediction_spearman_scores.columns.name = 'K-fold splits'

df_prediction_mae_scores.index.name = 'Repeats'
df_prediction_mae_scores.columns.name = 'K-fold splits'
...
print(df_prediction_spearman_scores)
print(df_prediction_mae_scores)
###############################################################################
# SAVE THE RESULTS
###############################################################################
