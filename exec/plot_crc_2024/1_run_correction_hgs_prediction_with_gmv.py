...
def plot_bar_with_scatter(data, x, y, corr_target, gender, n_regions_survived, color):
    ...
    plt.xlabel(x.capitalize(), fontsize=30, fontweight='bold')
...
# Females Correlations GMV vs True HGS
plot_bar_with_scatter(sorted_p_values_predicted_female, 'regions', 'correlations', "corrected_predicted", 'female', corrected_predicted_n_regions_survived_female, color="#f45f74")
# Males Correlations GMV vs True HGS
plot_bar_with_scatter(sorted_p_values_predicted_male, 'regions', 'correlations', "corrected_predicted", 'male', corrected_predicted_n_regions_survived_male, color="#00b0be")
...
# Females Correlations GMV vs True HGS
plot_bar_with_scatter(sorted_p_values_delta_female, 'regions', 'correlations', "predicted", 'female', predicted_n_regions_survived_female, color="#f45f74")
# Males Correlations GMV vs True HGS
plot_bar_with_scatter(sorted_p_values_delta_male, 'regions', 'correlations', "predicted", 'male', predicted_n_regions_survived_male, color="#00b0be")
...
# Females Correlations GMV vs True HGS
plot_bar_with_scatter(sorted_p_values_delta_female, 'regions', 'correlations', "corrected_delta", 'female', corrected_delta_n_regions_survived_female, color="#f45f74")
# Males Correlations GMV vs True HGS
plot_bar_with_scatter(sorted_p_values_delta_male, 'regions', 'correlations', "corrected_delta", 'male', corrected_delta_n_regions_survived_male, color="#00b0be")
...
for gender in ["female", "male"]:
    ...
    for cnfd_idx, cnfd in enumerate(list(df_tmp.columns)[:2]):
        ...
        # cerebellar SUID
        cer_corr = df_tmp.iloc[100:116, cnfd_idx-1].copy()
        cer_corr_newidx = cer_corr.copy()
        cer_corr_newidx.reset_index()
        # subcortical Tian
        sub_corr = df_tmp.iloc[116:, cnfd_idx-1].copy()
        ...
        # figure saving names
        folder_path = f"/data/project/stroke_ukb/knazarzadeh/project_hgsprediction/brain_imaging_data/brain_plots/{gender}/{brain_plot}"
