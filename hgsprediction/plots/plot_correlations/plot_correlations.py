...
def plot_hgs_correlations(df, x, y, stroke_cohort, feture_type, target, gender):
    
    if y == "actual-predicted":
        y = "hgs_" + "(" + y + ")"
        words = y.split('-')
        capitalized_words = [word.capitalize() for word in words]
        y_label = '-'.join(capitalized_words) + " HGS"
    else:
        y = "hgs_" + y
...
##############################################################################
    ...
    ax.set_xlabel(f"{x_label}", fontweight="bold", labelsize=20)
    ax.set_ylabel(f"{y_label}", fontweight="bold", labelsize=20)
    ...
    if gender == "both_gender":
        ax.set_title(f"{y_label} vs {x_label} - Features={feature_type}, Target={target} (N={len(df)}) /n \
                     (Females={len(df_female)}, Males={len(df_male)})", fontsize=15, fontweight="bold", y=1)
    elif gender == "female":
        ax.set_title(f"{y_label} vs {x_label} - Features={feature_type}, Target={target} (Females={len(df)})", fontsize=15, fontweight="bold", y=1)
    
    elif gender == "male":
        ax.set_title(f"{y_label} vs {x_label} - Features={feature_type}, Target={target} (Males={len(df)})", fontsize=15, fontweight="bold", y=1)
