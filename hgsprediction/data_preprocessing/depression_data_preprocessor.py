...
"""Perform different preprocessing on depression."""
...
###############################################################################
class DepressionMainDataPreprocessor:
...
###############################################################################
    def remove_missing_depression_dates(self, df):
        """ Drop all subjects who has no date of depression and
        ...
        df : pandas.DataFrame
            DataFrame of data specified.   
        """
        df = df[(~df['130894-0.0'].isna()) & (~df['130894-0.0'].isin(["1900-01-01", "1901-01-01", "1902-02-02", "1903-03-03", "1909-09-09", "2037-07-07"]))]
...
# between depression date and the attendence dates (for each session) 
# To see each subject had depression before/after depression
# to find the subject's depression cohort (Pre-, Post- or Longitudinal)
###############################################################################
    def define_followup_days(self, df):
        ...
        for ses in range(0, sessions):
            ...
            df.loc[:, f'followup_days-{ses}.0'] = (attendance_date-onset_date).dt.days

        return df

###############################################################################
    def preprocess_depression_df(self, df):
        """Use the depression dataframe. 
        ...
        post_sorted_columns.columns = [f"1st_{depression_cohort}_session",
                                        f"2nd_{depression_cohort}_session",
                                        f"3rd_{depression_cohort}_session",
                                        f"4th_{depression_cohort}_session"]
        ...
        # Combine selected rows from 'df' with sorted columns from 'sorted_columns' to create 'post_depression_visits_df'
        post_depression_visits_df = pd.concat([df[df.index.isin(post_sorted_columns.index)], post_sorted_columns], axis=1)
        ##################################################################        
        ############### Extract All Pre depression Subjects #################
        ##################################################################
        depression_cohort = "pre-depression"
        # Drop rows where values in all four columns of followup_days 
        # are greather than 0 (Drop All Post-depression subjects)
        # And keep rows with at least one value (< 0)
        ...
        # Set custom column names for the sorted_columns DataFrame
        pre_sorted_columns.columns = [f"1st_{depression_cohort}_session",
                                        f"2nd_{depression_cohort}_session",
                                        f"3rd_{depression_cohort}_session",
                                        f"4th_{depression_cohort}_session"]
        ...
        # Combine selected rows from 'df' with sorted columns from 'sorted_columns' to create 'pre_depression_visits_df'
        pre_depression_visits_df = pd.concat([df[df.index.isin(pre_sorted_columns.index)], pre_sorted_columns], axis=1)
        
        ##################################################################        
        ############### Extract Longitudinal depression Subjects #############
        ##################################################################
        # The intersection between pre and post dataframes of depression
        # will be the longitudinal dataframe.
        # Find duplicate index values
        duplicate_index = pre_depression_visits_df.index.intersection(post_depression_visits_df.index)
        # Find duplicate index values on the original DataFrame
        df_longitudinal_depression = df[df.index.isin(duplicate_index)]
        # List of new pre_visits column names
        pre_visits_columns = ["1st_pre-depression_session",
                              "2nd_pre-depression_session",
                              "3rd_pre-depression_session",
                              "4th_pre-depression_session"]
        # List of new post_visits column names
        post_visits_columns = ["1st_post-depression_session",
                               "2nd_post-depression_session",
                               "3rd_post-depression_session",
                               "4th_post-depression_session"]
        # Join pre_visits columns from df_all_pre_depression_visits into longitudinal_depression_df    
        df_longitudinal_depression = df_longitudinal_depression.join(pre_depression_visits_df[pre_depression_visits_df.index.isin(duplicate_index)][pre_visits_columns])
        # Join post_visits columns from df_all_post_depression_visits into longitudinal_depression_df
        df_longitudinal_depression = df_longitudinal_depression.join(post_depression_visits_df[post_depression_visits_df.index.isin(duplicate_index)][post_visits_columns])
        ##################################################################        
        ############### Merge All 3 types of sub-type depression #############
        ##################################################################
        # Merge All 3 types of sub-type depression dataframes 
        # while excluding duplicates from pre-depression and post-depression visits,
        # and include the longitudinal depression dataframe.
        merged_df = pd.concat([pre_depression_visits_df[~pre_depression_visits_df.index.isin(duplicate_index)],
                               post_depression_visits_df[~post_depression_visits_df.index.isin(duplicate_index)],
                               df_longitudinal_depression])
    ...
    def calculate_dominant_nondominant_hgs(self, df):
        ...
        for depression_cohort in ["pre-depression", "post-depression"]:
            for visit_session in range(1, 5):
                ...
                index = df[df.loc[:, session_column] == 0.0].index
                ...
                # ------------------------------------
                index = df[df.loc[:, session_column] == 3.0].index
                filtered_df = df.loc[index, :]
                idx = filtered_df[filtered_df.loc[:, "1707-0.0"] == 1.0].index
                ...
                df.loc[idx_tmp, hgs_nondominant] = df.loc[idx_tmp, ["46-2.0", "47-2.0"]].min(axis=1)
...
###############################################################################
    def validate_handgrips(self, df, session_column):
        ...
        if df[session_column].isna().sum() < len(df):
            ...
            # Calling the modules
            df = self.remove_missing_hgs_pre_post_depression(df, session_column)
            hgs_dominant = session_column.replace(substring_to_remove, "hgs_dominant")
            # ------------------------------------
...
###############################################################################
    def remove_missing_hgs_pre_post_depression(self, df, session_column):
...
###############################################################################
    def extract_post_depression_df(self, df, mri_status):
        """Extract the post depression dataframe from the depression dataframe. 
        ...
        # for only Post-depression subjects
        post_depression_df = df[mask]
        
        return post_depression_df

###############################################################################
    def extract_pre_depression_df(self, df, mri_status):
        """Extract the pre depression dataframe from the depression dataframe. 
        ...
        # Apply the mask to the DataFrame to get the desired rows
        # for only Pre-depression subjects
        pre_depression_df = df[mask]
        
        return pre_depression_df

###############################################################################
    def extract_longitudinal_depression_df(self, df, mri_status):
        """Extract the longitudinal dataframe from the depression dataframe. 
        ...
        df : pandas.DataFrame
            DataFrame of data specified.
        """
        df_pre_depression = self.extract_pre_depression_df(df, mri_status)
        df_post_depression = self.extract_post_depression_df(df, mri_status)

        # The intersection between pre and post dataframes of depression
        # will be the longitudinal dataframe.
        # Find duplicate index values
        merged_df = pd.concat([df_pre_depression, df_post_depression])
        # Find duplicate index values on the original DataFrame
        longitudinal_depression_df = df[~df.index.isin(merged_df.index)]
        
        return longitudinal_depression_df
...
###############################################################################
class depressionValidateDataPreprocessor:
    def __init__(self, 
                 df, 
                 mri_status,
                 depression_cohort, 
        ...
        self.depression_cohort = depression_cohort
        self.visit_session = visit_session
        
        assert isinstance(df, pd.DataFrame), "df must be a dataframe!"
        assert isinstance(mri_status, str), "mri_status must be a string!"
        assert isinstance(depression_cohort, str), "depression_cohort must be a string!"
        assert isinstance(visit_session, int), "visit_session must be a string!"

        if visit_session == 1:
            self.session_column = f"1st_{depression_cohort}_session"
        elif visit_session == 2:
            self.session_column = f"2nd_{depression_cohort}_session"
        elif visit_session == 3:
            self.session_column = f"3rd_{depression_cohort}_session"
        elif visit_session == 4:
            self.session_column = f"4th_{depression_cohort}_session"
################################ EXTRACT DATA ##############################
# The main goal of data validation is to verify that the data is 
# accurate, reliable, and suitable for the intended analysis.
###############################################################################
...
###############################################################################
    def validate_handgrips(self, df):
        ...
        if df[session_column].isna().sum() < len(df):
            # Calculate Dominant HGS by
            # Calling the modules
            df = self.remove_missing_hgs_pre_post_depression(df)
...
###############################################################################
    def remove_missing_hgs_pre_post_depression(self, df):
