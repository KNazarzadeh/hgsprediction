...
for stroke_cohort in ["pre-stroke", "post-stroke"]:
    for visit_session in range(1, 4):
        ...
        if len(extracted_pre_post_data) > 0:
            ...
            stroke_save_data.save_validated_hgs_data(hgs_validated_df, population, mri_status, session_column, stroke_cohort, "both_gender")
            stroke_save_data.save_validated_hgs_data(hgs_validated_df_female, population, mri_status, session_column, stroke_cohort, "female")
            stroke_save_data.save_validated_hgs_data(hgs_validated_df_male, population, mri_status, session_column, stroke_cohort, "male")

            stroke_save_data.save_original_extracted_pre_post_data(extracted_pre_post_data, population, mri_status, session_column, stroke_cohort, "both_gender")
            stroke_save_data.save_original_extracted_pre_post_data(extracted_pre_post_data_female, population, mri_status, session_column, stroke_cohort, "female")
            stroke_save_data.save_original_extracted_pre_post_data(extracted_pre_post_data_male, population, mri_status, session_column, stroke_cohort, "male")
        else:
