...
df = df[df['patient_id'].isin(df_motor_lesions.index)]

if lesion_side == "right":
    dominant_side = "left"
    df = df[df['hgs_dominant_side']=='left']

elif lesion_side == "left":
    dominant_side = "right"
    df = df[df['hgs_dominant_side']=='right']
print("===== Done! =====")
embed(globals(), locals())
##############################################################################
