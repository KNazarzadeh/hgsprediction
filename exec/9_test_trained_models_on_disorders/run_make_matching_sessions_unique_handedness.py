...
df_disorder_main = load_disorder_corrected_prediction_results(
    population,
    mri_status,
    session_column,
    model_name,
...
df_disorder_main.index.name = "subjectID"
df_disorder_main.loc[:, "disorder"] = 1

first_event = first_event + "_handedness"
# print("===== Done! =====")
# embed(globals(), locals())
###############################################################################
...
not_same_values = df_control_matched_main[df_control_matched_main[f'1st_pre-{population}_patient_id'] != df_control_matched_main[f'1st_post-{population}_patient_id']]

if not_same_values.empty:
    print("pre and post controls are for the same paitent id")

print(df_control_matched_main)
print(df_disorder_main)
if df_control_matched_main[df_control_matched_main.index.duplicated()].empty:
...
# Adding a new index'
age_mean_disorder = df_disorder_main[f"1st_pre-{population}_age"].mean()
age_mean_control = df_control_matched_main[f"1st_pre-{population}_age"].mean()
...
bmi_mean_disorder = df_disorder_main[f"1st_pre-{population}_bmi"].mean()
bmi_mean_control = df_control_matched_main[f"1st_pre-{population}_bmi"].mean()
...
height_mean_disorder = df_disorder_main[f"1st_pre-{population}_height"].mean()
height_mean_control = df_control_matched_main[f"1st_pre-{population}_height"].mean()
...
WHR_mean_disorder = df_disorder_main[f"1st_pre-{population}_waist_to_hip_ratio"].mean()
WHR_mean_control= df_control_matched_main[f"1st_pre-{population}_waist_to_hip_ratio"].mean()
...
HGS_mean_disorder = df_disorder_main[f"1st_pre-{population}_{target}"].mean()
HGS_mean_control= df_control_matched_main[f"1st_pre-{population}_{target}"].mean()
HGS_difference = HGS_mean_disorder - HGS_mean_control
...
# Adding a new index'
age_mean_disorder = df_disorder_main[f"1st_post-{population}_age"].mean()
age_mean_control = df_control_matched_main[f"1st_post-{population}_age"].mean()
# Calculate the difference between the mean ages
age_difference = age_mean_disorder - age_mean_control
df_check_matching_post.loc['age_mean'] = [age_mean_disorder, age_mean_control, age_difference]

bmi_mean_disorder = df_disorder_main[f"1st_post-{population}_bmi"].mean()
bmi_mean_control = df_control_matched_main[f"1st_post-{population}_bmi"].mean()
bmi_difference = bmi_mean_disorder - bmi_mean_control
df_check_matching_post.loc['bmi_mean'] = [bmi_mean_disorder, bmi_mean_control, bmi_difference]

height_mean_disorder = df_disorder_main[f"1st_post-{population}_height"].mean()
height_mean_control = df_control_matched_main[f"1st_post-{population}_height"].mean()
height_difference = height_mean_disorder - height_mean_control
df_check_matching_post.loc['height_mean'] = [height_mean_disorder, height_mean_control, height_difference]

WHR_mean_disorder = df_disorder_main[f"1st_post-{population}_waist_to_hip_ratio"].mean()
WHR_mean_control= df_control_matched_main[f"1st_post-{population}_waist_to_hip_ratio"].mean()
WHR_difference = WHR_mean_disorder - WHR_mean_control
df_check_matching_post.loc['WHR_mean'] = [WHR_mean_disorder, WHR_mean_control, WHR_difference]

HGS_mean_disorder = df_disorder_main[f"1st_post-{population}_{target}"].mean()
HGS_mean_control= df_control_matched_main[f"1st_post-{population}_{target}"].mean()
HGS_difference = HGS_mean_disorder - HGS_mean_control
df_check_matching_post.loc['HGS_mean'] = [HGS_mean_disorder, HGS_mean_control, HGS_difference]
...
sns.kdeplot(data=df_disorder_main, x=f"1st_pre-{population}_propensity_scores", fill=True, color=disorder_color, ax=ax[0], label=f'{population.capitalize()}\n{gender.capitalize()}(N={len(df_disorder_main)})')

sns.kdeplot(data=df_control_matched_main, x=f"1st_pre-{population}_propensity_scores", fill=True, color="grey", ax=ax[1], label='Control')
sns.kdeplot(data=df_disorder_main, x=f"1st_pre-{population}_propensity_scores", fill=True, color=disorder_color, ax=ax[1], label=f'{population.capitalize()}\n{gender.capitalize()}(N={len(df_disorder_main)})')
