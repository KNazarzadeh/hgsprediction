...
x_axis = ["age", "bmi", "height", "waist_to_hip_ratio"]
for stroke_subgroup in ["pre-stroke", "post-stroke"]:
    ...
    for i in ["actual", "predicted"]:
        # Set weights based on correlation coefficients
        weights_df = df_corr.loc[[col for col in df_corr.index if col.endswith(i)], :]
        weights = weights_df.values.tolist()
        print(weights)
        # Set variable labels
        labels = ['Age', 'BMI', 'Height', 'Waist-to-Hip Ratio']

        # Set variable values for the sample population
        values = df_merged[[col for col in df_merged.columns if any(item in col for item in weights_df.columns)]].median().to_list()
        print(values)

        # Plot the sample population
        ax.plot(np.linspace(0, 2 * np.pi, len(labels), endpoint=False), values, color='b', marker='o')
        ax.fill(np.linspace(0, 2 * np.pi, len(labels), endpoint=False), values, color='b', alpha=0.25)

        # Plot the weights
        ax.plot(np.linspace(0, 2 * np.pi, len(labels), endpoint=False), weights[0], color='r', marker='o')

        # Add labels and title
        ax.set_xticks(np.linspace(0, 2 * np.pi, len(labels), endpoint=False))
        ax.set_xticklabels(labels)
    ax.set_title(f'{stroke_subgroup} - HGS')

    # Show the plot
    plt.show()
    plt.savefig(f"{stroke_subgroup}_{i}_radar.png")
