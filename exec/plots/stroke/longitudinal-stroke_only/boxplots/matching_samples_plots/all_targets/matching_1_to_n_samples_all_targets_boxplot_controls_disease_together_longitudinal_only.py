...
for y_axis in ["delta"]:
    ...
    # Initialize a list to store the test results
    results = pd.DataFrame(columns=["hgs_target_stroke_cohort", "ranksum_stat", "ranksum_p_value", f"max_sample_{y_axis}", f"max_stroke_{y_axis}"])
    for i, hgs_target_stroke_cohort in enumerate(["HGS Left-pre", "HGS Right-pre", "HGS L+R-pre"]):
        tmp = melted_df[melted_df["hgs_target_stroke_cohort"]== hgs_target_stroke_cohort]
        tmp_samples = tmp[tmp["disease"]==0]
        tmp_stroke = tmp[tmp["disease"]==1]
        stat, p_value = ranksums(tmp_samples["value"], tmp_stroke["value"])
    ...
    # Define a custom palette with two blue colors
    custom_palette = sns.color_palette(['#40B0A6', 'lightpink'])  # You can use any hex color codes you prefer
    ...
    plt.show()
    plt.savefig(f"pre_boxplot_1_to_{n}_samples_{session_column}_{y_axis}_{population}_{feature_type}_hgs_both_gender_controls_Stroke.png")
    plt.close()
    ###############################################################################
...
for y_axis in ["delta"]:
    ...
    # Define a custom palette with two blue colors
    custom_palette = sns.color_palette(['#117733', '#D35FB7'])  # You can use any hex color codes you prefer
    plt.figure(figsize=(8, 8))  # Adjust the figure size if needed
    sns.set(style="whitegrid")
    # Define the order in which you want the x-axis categories
    ...
    plt.show()
    plt.savefig(f"post_boxplot_1_to_{n}_samples_{session_column}_{y_axis}_{population}_{feature_type}_hgs_both_gender_controls_Stroke.png")
    plt.close()
print("===== Done! =====")
embed(globals(), locals())